<?php

namespace app\common\searchs;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\common\entity\BookSystemConfigEntity;

/**
 * BookSystemConfigSearch represents the model behind the search form of `app\common\entity\BookSystemConfigEntity`.
 */
class BookSystemConfigSearch extends BookSystemConfigEntity
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['config_id'], 'integer'],
            [['config_name', 'config_value', 'config_desc', 'create_on', 'update_on'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = BookSystemConfigEntity::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'config_id' => $this->config_id,
            'create_on' => $this->create_on,
            'update_on' => $this->update_on,
        ]);

        $query->andFilterWhere(['like', 'config_name', $this->config_name])
            ->andFilterWhere(['like', 'config_value', $this->config_value])
            ->andFilterWhere(['like', 'config_desc', $this->config_desc]);

        return $dataProvider;
    }
}
